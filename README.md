# Neovim

## Homebrew

```sh
# Pin (freeze) neovim formula
brew pin neovim

# List pinned formulas
brew list --pinned

# Manual update should work as epxected
brew upgarde neovim
```

## Install Tools and Language Servers

```sh
brew install efm-langserver go gopls harper node

npm i -g @github/copilot pnpm prettier serve typescript yarn

npm i -g @biomejs/biome @tailwindcss/language-server @vtsls/language-server bash-language-server cssmodules-language-server gh-actions-language-server tombi vscode-langservers-extracted yaml-language-server
```
